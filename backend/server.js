...
const mapUser = (row) => {
    ...
    return {
        ...
        businessCategory: row.business_category, // Fixed casing match
...
router.post('/auth/register', async (req, res) => {
  ...
  try {
    ...
    // Email sending logic omitted for brevity, essentially same as before
    res.status(201).json(safeUser);
  } catch (error) { res.status(500).json({ message: 'Registration failed. ' + error.message }); }
});

// --- USER MANAGEMENT (Using Query Params for Single User to support IDs with slashes) ---
...
// Legacy support for /users/:id (Warning: Breaks with slashes)
router.get('/users/:id', async (req, res) => {
    // This is kept for backward compatibility but /user?id=... is preferred
...
// --- PAYMENTS (Unified GET to support Query Params) ---
...
// --- MESSAGES (Fixed for ID Slashes) ---

// 1. Get Chat History (Uses Query Params)
router.get('/messages/chat', async (req, res) => {
    const { userId, otherUserId } = req.query;
    if (!userId || !otherUserId) return res.status(400).json({ message: "Missing userId or otherUserId query params" });
...
// 2. Get Conversation List (Uses Query Params)
router.get('/messages/conversations', async (req, res) => {
    ...
    try {
        // Raw fetch of messages involving user
        const messagesQuery = `
        ...
        // Javascript aggregation to handle ID intricacies
...
// 3. Mark Read (Uses Body to avoid slash issues in URL)
...
// 4. Get Unread Count (Uses Query Params)
